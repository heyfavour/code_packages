...
class Nstep_Memory(object):
    ...
    def sample(self, batch_size=BATCH_SIZE):
        if self.n_multi_step == 1:
            ...
            print(self.memory[sample_index,sample_index+2])
            return
...
if __name__ == '__main__':
    m = Nstep_Memory(memory_size=100,n_multi_step=2)
    for i in range(100):
        m.store_transition((np.array([i, 2, 3, 4]), 10 + i, 100 + i, np.array([10 + i, 8, 9, 10]),True if i %5 ==0 else False ))
    # print(m.memory)
    # print("--------------")
    print(m.sample(7))
